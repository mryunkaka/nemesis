...
const { collectLiveSyncIds, listLiveSyncFiles } = require("./live-sync");
...
// Region type lookup based on official data from inaproc.id
// Used when GeoJSON doesn't have TYPE_2 field
const REGION_TYPE_LOOKUP = {
  "kotabaru": "Kabupaten",
  "kotawaringin barat": "Kabupaten",
  "kotawaringin timur": "Kabupaten",
  "donggala": "Kabupaten",
...
// Override incorrect region types in GeoJSON to match official data from inaproc.id
const REGION_TYPE_OVERRIDES = {
  "kotabaru": "Kabupaten",
  "kotawaringin barat": "Kabupaten",
  "kotawaringin timur": "Kabupaten",
};
...
const REGION_KEY_ALIASES = {
  ...
  // Fix for regions showing 0 data - handle "Kota X" that becomes "X" after prefix removal
  "kotabaru": "kotabaru",
  "kota baru": "kotabaru",
  "baru": "kotabaru",  // When "Kota Baru" becomes "baru" after prefix removal
  "kotawaringin timur": "kotawaringintimur",
  "kota waringin timur": "kotawaringintimur",
  "waringin timur": "kotawaringintimur",
  "kotawaringin barat": "kotawaringinbarat",
  "kota waringin barat": "kotawaringinbarat",
  "waringin barat": "kotawaringinbarat",
  "toba": "tobasamosir",
  ...
  "donggala": "donggala",
  "dongala": "donggala",  // Typo in source data
  "bolaang mongondow": "bolaangmongondow",
  "mongondow": "bolaangmongondow",
  "bolaang mengondow": "bolaangmongondow",  // Typo in source data
  "sangihe talaud": "kepulauan-sangihe",  // Merged region in source data
...
function resolveRegionTypeFromName(value, fallbackType = "Kota") {
  const normalized = toComparableWords(value);

  if (REGION_TYPE_OVERRIDES[normalized]) {
    return REGION_TYPE_OVERRIDES[normalized];
  }

  if (REGION_TYPE_LOOKUP[normalized]) {
    return REGION_TYPE_LOOKUP[normalized];
  }
...
function selectAuditSource() {
  ...
  const supplementalJsonlFiles = listLiveSyncFiles("jsonl");
  const supplementalIds = collectLiveSyncIds();
...
function forEachAuditRow(source, onRow) {
  const shouldSkipBaseRow = (row) => {
    if (!source.supplementalIds || !source.supplementalIds.size) {
      return false;
    }

    const rowId = cleanText(row && row.id);
    return Boolean(rowId) && source.supplementalIds.has(String(rowId).trim());
  };
...
function createLegacyRegionRecord(feature, index) {
  const provinceName = normalizeProvinceDisplayName(feature.properties.NAME_1 || feature.properties.WADMPR);
  let regionType = feature.properties.TYPE_2 ? normalizeRegionType(feature.properties.TYPE_2) : null;
  const rawRegionName = cleanText(feature.properties.NAME_2 || feature.properties.WADMKK) || "";
  const gid = cleanText(feature.properties.GID_2);

  // If TYPE_2 is missing, use lookup table based on region name (from inaproc.id)
  if (!regionType) {
    regionType = resolveRegionTypeFromName(rawRegionName, "Kota");
  } else {
    regionType = resolveRegionTypeFromName(rawRegionName, regionType);
  }

  const regionName = normalizeRegionDisplayName(rawRegionName, regionType);
...
function normalizeDistrictRegionType(value) {
  const normalized = toComparableWords(value);
  const inferredType = normalized.startsWith("kota") ? "Kota" : "Kabupaten";
  return resolveRegionTypeFromName(value, inferredType);
}
...
function resolveRegionKeys(locationRaw, lookup) {
  ...
  for (const segment of splitLocationSegments(locationRaw)) {
    ...
    // Fallback: try alternative region type for known mismatches
    // Some source data has incorrect region type (Kota vs Kabupaten)
    const alternativeType = parsed.regionType === "Kota" ? "Kabupaten" : "Kota";
    const fallbackKey = buildLocationLookupKey(parsed.provinceName, parsed.regionName, alternativeType);
    const fallbackRegion = lookup.get(fallbackKey);

    if (fallbackRegion) {
      resolvedKeys.add(fallbackRegion.region_key);
    }
  }
...
function resolveProvinceKeys(locationRaw, provinceLookup, regionLookup) {
  ...
  for (const segment of splitLocationSegments(locationRaw)) {
    ...
    // Fallback: try alternative region type for known mismatches
    const alternativeType = parsed.regionType === "Kota" ? "Kabupaten" : "Kota";
    const fallbackRegion = regionLookup.get(buildLocationLookupKey(parsed.provinceName, parsed.regionName, alternativeType));

    if (fallbackRegion) {
      const provinceFromRegion = provinceLookup.get(buildProvinceLookupKey(fallbackRegion.province_name));
      if (provinceFromRegion) {
        resolvedKeys.add(provinceFromRegion.province_key);
      }
    }
