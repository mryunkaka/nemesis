...
const { createDataRefreshService } = require("./data-refresh");
...
if (!hasSchema) {
  ...
  console.error(`Run "npm run db:reset" to rebuild the database from seed data.`);
  db.close();
  process.exit(1);
}
...
const dataRefreshService = createDataRefreshService(db);
const app = createApp(db, { dataRefreshService });
